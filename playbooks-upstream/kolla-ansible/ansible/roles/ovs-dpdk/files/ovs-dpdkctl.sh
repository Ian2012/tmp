...
function bind_nics {
    for nic in $(list_dpdk_nics); do
        ...
        if [ "$device_address" == "null" ]; then
            bond_members="$(get_value $nic members | tr ',' ' ')"
            for member in $bond_members; do
                member_addr="$(get_value $member address)"
                member_current_driver="$(get_driver_by_address $member_addr)"
                member_target_driver="$(get_value $member driver)"
                if [ "$member_current_driver" != "$member_target_driver" ]; then
                    set_value $member old_driver $member_current_driver
                    unbind_nic $member_addr $member_current_driver
                    bind_nic $member_addr $member_target_driver
                fi
            done
            continue
        fi
...
function unbind_nics {
    for nic in $(list_dpdk_nics); do
        if [ "$(get_value $nic address)" == "null" ]; then
            bond_members="$(get_value $nic members | tr ',' ' ')"
            for member in $bond_members; do
                if [ "$(is_set $member old_driver)" == 0 ]; then
                    member_addr="$(get_value $member address)"
                    member_current_driver="$(get_driver_by_address $member_addr)"
                    member_target_driver="$(get_value $member old_driver)"
                    if [ "$member_current_driver" != "$member_target_driver" ]; then
                        unbind_nic $member_addr $member_current_driver
                        bind_nic $member_addr $member_target_driver
                        del_value $member old_driver
                    fi
                fi
            done
        elif [ "$(is_set $nic old_driver)" == 0 ]; then
            device_address="$(get_value $nic address)"
            current_driver="$(get_driver_by_address $device_address)"
...
function init_ovs_db {
    ...
    other_config:dpdk-extra="--iova-mode=pa --proc-type primary $(get_value ovs pci_whitelist) " \
    other_config:pmd-rxq-assign="$(get_value ovs pmd_rxq_assign)" \
    other_config:pmd-sleep-max="$(get_value ovs pmd_sleep_max)"
}
...
function init_ovs_interfaces {
    ...
    for pair in  $pci_port_pairs; do
        ...
        if [ "$addr" == "null" ]; then
            bond_options="$(get_value ovs bond_options)"
            bond_members="$(get_value $nic members | tr ',' ' ')"
            set_interfaces_command=''
            for member in $bond_members; do
                member_addr="$(get_value $member address)"
                set_member_interface_command="-- set Interface $member type=dpdk options:dpdk-devargs=$member_addr mtu_request=9000"
                set_interfaces_command="$set_interfaces_command $set_member_interface_command"
            done
            ovs-vsctl add-bond $bridge $nic $bond_members $bond_options $set_interfaces_command
            # Enable LLDP in each bond member interface
            for member in $bond_members; do
                interface_extra_options="$(get_value $member interface_extra_options)"
                if [ ! -z "$interface_extra_options" ]; then
                    ovs-vsctl set interface $member $interface_extra_options
                fi
            done
        # ovs 2.6 and older requires dpdkX names, ovs 2.7+ requires dpdk-devargs instead.
        elif [ "$(get_value ovs physical_port_policy)" == "indexed" ]; then
            ...
            -- set Interface  $nic type=dpdk options:dpdk-devargs=$addr mtu_request=9000
            # Enable LLDP in the interface
            interface_extra_options="$(get_value $nic interface_extra_options)"
            if [ ! -z "$interface_extra_options" ]; then
                ovs-vsctl set interface $nic $interface_extra_options
            fi
        fi
    ...
    # Tags tunnel_interface ports with assigned VLANs
    raw_tunnel_vlan_mappings=$(get_value ovs tunnel_vlan_mappings)
    tunnel_vlan_mappings=( ${raw_tunnel_vlan_mappings//,/ } )
    for pair in "${tunnel_vlan_mappings[@]}"; do
        tunnel_interface=`echo $pair | cut -f 1 -d ":"`
        tunnel_vlan=`echo $pair | cut -f 2 -d ":"`
        ovs-vsctl set port $tunnel_interface tag=$tunnel_vlan
    done
